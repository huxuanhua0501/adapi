...
public class HttpClientUtil {
	...
	private final static int CONNECT_TOTAL = 2000;// 最大连接数
	
	private final static int CONNECT_ROUTE = 500;// 每个路由基础的连接数
	
	private final static int CONN_MANAGER_TIMEOUT = 1000; //连接不够用的时候等待超时时间
...
//			System.out.println("now client pool " + connManager.getTotalStats().toString());
	...
	*/
	...
	public static byte[] sendBaiduPost(String reqURL, byte[] bytes) {
		...
		// ENCODE_CHARSET );
		httpPost.setHeader(HTTP.CONTENT_TYPE,"application/x-protobuf; charset=" + ENCODE_CHARSET);
		...
		try {
			...
			if (null != entity) {
				byt = EntityUtils.toByteArray(entity);
			}
		} catch (Exception e) {
			logger.error("请求广告通信[" + reqURL + "]时偶遇异常,堆栈轨迹如下", e);
		} finally {
			try {
				if (response != null)
					response.close();
	...
	@SuppressWarnings("finally")
	public static int sendBaiduGet(String reqURL) {
		...
		try {
			...
			logger.error("广告回调通信[" + reqURL + "]时偶遇异常,堆栈轨迹如下", e);
		} finally {
			try {
				if (response != null)
					response.close();
